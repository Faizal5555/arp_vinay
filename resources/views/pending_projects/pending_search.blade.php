    ...
    <tr class="project-row">
        <td>
            ...
            <input type="date" name="entry_date[]" class="form-control entry-date"
                value="{{ $project->entry_date ? \Carbon\Carbon::parse($project->entry_date)->format('Y-m-d') : '-' }}"
                readonly>
        </td>
        ...
        <td>
            <select name="fy[]" class="form-select">
                ...
                @for ($i = 10; $i <= 50; $i++)
                    ...
                    @endphp
                    <option value="{{ $fy }}" {{ $project->fy == $fy ? 'selected' : '' }}>{{ $fy }}
                    </option>
                @endfor
            </select>
        </td>
        ...
        <td>
            <select name="quarter[]" class="form-select">
                ...
                @foreach (['Q1', 'Q2', 'Q3', 'Q4'] as $q)
                    <option value="{{ $q }}" {{ $project->quarter == $q ? 'selected' : '' }}>
                        {{ $q }}</option>
                @endforeach
            </select>
        </td>
        ...
        <td><input type="text" name="pn_no[]" class="form-control" value="{{ $project->pn_no }}"></td>
        ...
        <td><input type="text" name="margin[]" class="form-control" value="{{ $project->margin }}"></td>
        <td><input type="text" name="final_invoice_amount[]" class="form-control"
                value="{{ $project->final_invoice_amount }}"></td>
        <td><input type="text" name="comments[]" class="form-control" value="{{ $project->comments }}"></td>
        ...
        <td>
            ...
            <textarea name="partial_comment[]" class="mt-2 form-control partialCommentBox fw-bold"
                style="display: {{ $project->invoice_status === 'partial' ? 'block' : 'none' }};" rows="2"
